...
class IPFSClient:
    ...
    def _initialize_encryption(self, encrypt_by_default: Optional[bool], encryption_key: Optional[bytes]):
            ...
            if env_key:
                try:
                    ...
                    if len(self.encryption_key) != nacl.secret.SecretBox.KEY_SIZE:
                        print(f"Warning: Encryption key from .env has incorrect length. Expected {nacl.secret.SecretBox.KEY_SIZE} bytes, got {len(self.encryption_key)} bytes.")
                        self.encryption_key = None
        ...
        self.encryption_available = (
            ...
            self.encryption_key is not None and 
            len(self.encryption_key) == nacl.secret.SecretBox.KEY_SIZE
        )
        
        # If encryption is requested but not available, warn the user
        if self.encrypt_by_default and not self.encryption_available:
            print(f"Warning: Encryption requested but not available. Check that PyNaCl is installed and a valid encryption key is provided.")
    ...
    def encrypt_data(self, data: bytes) -> bytes:
        ...
        if not self.encryption_available:
            raise ValueError("Encryption is not available. Check that PyNaCl is installed and a valid encryption key is provided.")
    ...
    def decrypt_data(self, encrypted_data: bytes) -> bytes:
        ...
        try:
            ...
            raise ValueError(f"Decryption failed: {str(e)}. Incorrect key or corrupted data?")
    ...
    def upload_file(self, file_path: str, include_formatted_size: bool = True, encrypt: Optional[bool] = None) -> Dict[str, Any]:
        """
        ...
        if should_encrypt and not self.encryption_available:
            raise ValueError("Encryption requested but not available. Check that PyNaCl is installed and a valid encryption key is provided.")
    ...
    def upload_directory(self, dir_path: str, include_formatted_size: bool = True, encrypt: Optional[bool] = None) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                for root, _, files in os.walk(dir_path):
                    for file in files:
                        ...
                        temp_file_dir = os.path.dirname(os.path.join(temp_dir, rel_path))
    ...
    def download_file(self, cid: str, output_path: str, decrypt: Optional[bool] = None) -> Dict[str, Any]:
        ...
        if should_decrypt and not self.encryption_available:
            raise ValueError("Decryption requested but not available. Check that PyNaCl is installed and a valid encryption key is provided.")
