...
def generate_key():
    ...
    if not ENCRYPTION_AVAILABLE:
        print("Error: PyNaCl is required for encryption. Install it with: pip install pynacl")
        sys.exit(1)
...
def key_generation_cli():
    ...
    parser = argparse.ArgumentParser(description="Generate a secure encryption key for Hippius SDK")
    ...
    if args.copy:
        try:
            ...
            print("Warning: Could not copy to clipboard. Install pyperclip with: pip install pyperclip")
    ...
    print("client = HippiusClient(encrypt_by_default=True, encryption_key=encryption_key)")
...
def handle_credits(client, account_address):
    ...
    try:
        ...
        print(f"Account address: {account_address or client.substrate_client._keypair.ss58_address}")
    except Exception as e:
...
def handle_files(client, account_address, debug=False, show_all_miners=False):
    ...
    try:
        ...
        if files:
            ...
            print(f"No files found for account: {account_address or client.substrate_client._keypair.ss58_address}")
    except Exception as e:
...
"""
    ...
    keygen_parser = subparsers.add_parser("keygen", help="Generate an encryption key for secure file storage")
    keygen_parser.add_argument("--copy", action="store_true",
                             help="Copy the generated key to the clipboard")
