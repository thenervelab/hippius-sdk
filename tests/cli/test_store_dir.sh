#!/bin/bash
# Test script focused on the store-dir command and the --no-publish flag
...
# Test erasure-code with --no-publish for comparison
run_test "erasure-code with --no-publish" "$PYTHON -m hippius_sdk.cli erasure-code test_dir/test1.txt --no-publish"
